...
interface NowPlayResponse {
  results: NowPlayMovie[];
  page: number;
  total_pages: number;
}
...
interface PaginatedNowPlayResponse {
  results: NowPlayMovie[];
  nextPage: number | null;
}

async function fetchNowPlayContent({
  pageParam = 1,
  contentType
}: {
  pageParam?: number;
  ...
  const data: NowPlayResponse = await res.json();
  return {
...
export default function NowPlaying({ contentType }: NowPlayProps) {
  const {
    data,
    isLoading,
    isError,
    ...
    hasNextPage,
    isFetchingNextPage
  } = useInfiniteQuery<PaginatedNowPlayResponse>({
    queryKey: ["NowPlay", contentType],
    queryFn: ({ pageParam = 1 }) =>
      fetchNowPlayContent({ pageParam, contentType }),
    initialPageParam: 1,
